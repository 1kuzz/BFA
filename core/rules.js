...
export function scoreForm(ctx, RULES) {
  ...
  if (RULES.requireConsentVersion && !cv) recs.push('Добавить UF_CRM_CONSENT_VERSION');
  if (RULES.requireVisitorId && !ctx.hasVisitorId) recs.push('Добавить VisitorID (validator.js)');
  ...
  if (RULES.requireEmail && !ctx.hasEmail) recs.push('Добавить обязательный Email');
  var redirectFlagged = (RULES.requireHttpsRedirect && ctx.redirectIssue.indexOf('HTTP') > -1) ||
    (ctx.redirectIssue && ctx.redirectIssue.indexOf('HTTP') === -1);
  if (redirectFlagged) recs.push('Исправить редирект: ' + ctx.redirectIssue);
